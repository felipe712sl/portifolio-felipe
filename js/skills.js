...
(function () {
      ...
      if (entries[0].isIntersecting) {
        document.querySelectorAll('.skill-item').forEach(item => {
          ...
          const filled = segs.length
            ? Math.round((parseInt(item.querySelector('[data-active]')
                ?.closest('.skill-item')
                ?.querySelector('.skill-item__bar [data-active="true"]:last-of-type')
                ?.dataset.index || '0') + 1) / TOTAL_SEGMENTS * 100)
            : 0;
  ...
  (function () {
    ...
    function show(item, e) {
      ...
      entriesEl.innerHTML = exp.split('|').map(e => `
